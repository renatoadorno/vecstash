...
**D16 — `sqlite-vector-rs` foi descartado. Vetores ficam em coluna BLOB `f32` e o cosseno é calculado em Rust.**
Alternativa descartada: `sqlite-vector-rs` (D2), distribuindo o `.dylib` ao lado do binário.
Por quê: **descoberto na implementação.** A feature `library` da crate não registra nada em processo — `register()` procura um `libsqlite_vector_rs.dylib` no disco (via `SQLITE_VECTOR_RS_LIB`, ao lado do executável, ou em `target/`) e o carrega como extensão dinâmica do SQLite. O registro in-process existe só pela via `#[sqlite3_ext_main]`, que recebe uma `sqlite3_ext::Connection`, não uma `rusqlite::Connection`. Usar a crate exigiria distribuir um `.dylib` junto, quebrando o critério 3 do objetivo. Como D9 já decidira busca exata, nada se perde: o cosseno sobre vetores normalizados é um produto interno de ~30 linhas. Esta é a saída que a própria spec pré-autorizou na seção "Riscos a vigiar". `usearch` continua disponível quando o corpus exigir ANN, e os metadados nunca dependeram da tabela virtual.

**D17 — O modelo padrão passa a ser `bge-m3` fp16 (1,13GB), não int8.**
Alternativa descartada: int8/`model_quantized.onnx` (570MB), decidido em D4.
Por quê: **medido na implementação.** O teste de paridade contra os vetores do Python reprovou com int8 — pior similaridade de cosseno **0,981939**, muito abaixo do limiar de 0,999 do critério 2. Com fp16 o mesmo teste dá **0,999999**, o que também prova que o pipeline (tokenização, CLS pooling, normalização L2) está correto e que a perda era inteiramente da quantização. O custo medido no M1: ingestão de 3 arquivos passa de 8,0s para 16,4s e a busca de 1,8s para 3,1s, porque o ONNX Runtime em CPU converte fp16 para fp32 a cada operação. A troca é deliberada: 2x de tempo em favor da qualidade de busca, que é a razão de existir da ferramenta. `model_quantized.onnx` segue disponível via `model.onnx_file` para quem preferir velocidade.
